...
from app.planner import plan_single_trailer, plan_superlink
...
# Allow frontend to access API (even if served from same origin, good practice)
app.add_middleware(
    CORSMiddleware,
    allow_origins=["*"],
    allow_methods=["*"],
...
# We'll mount the 'static' directory.
...
async def generate_plan(request: PlanRequest):
    ...
    # Check if it's a Superlink (contains 'link' or we check type)
    if config and config.get('is_link'):
        # Map the config type to the exact key used in the library
        # The user selected e.g. "Tri-Axle Superlink"
